...
function extractCallsigns(message) {
  ...
  if (parts[0] === 'CQ') {
    ...
    const callIdx = parts[1] && /^[A-Z]/.test(parts[1]) && !/^\d/.test(parts[1]) && parts[1].length <= 3 ? 2 : 1;
...
module.exports = { WsjtxClient, extractCallsigns, MSG, BinaryWriter, buildHeader, encodeHeartbeat, encodeLoggedAdif };
