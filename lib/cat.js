...
class RigctldClient extends EventEmitter {
  ...
  tune(frequencyHz, mode, { split, filterWidth } = {}) {
    ...
    // Re-send mode AFTER frequency — Yaesu (and some other radios) recall the
    // last-used mode per band when the frequency crosses a band boundary,
    // overriding the mode we just set.
    if (modeToken) {
      this._write(`M ${modeToken} ${modePassband}\n`);
    }
    // Sending the Frequency one more time to handle any CW/SSB offset nonsense - W3AVP
    this._write(`F ${frequencyHz}\n`);
