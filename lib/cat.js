...
class CatClient extends EventEmitter {
  ...
  sendCwText(text) {
    ...
    if (this._isYaesu() && this._kyMode === 'km') {
      // FTDX101D/MP: write to Text Memory 1 via KM, then play back via KY6
      // Max 50 chars per memory slot; chunk longer messages
      for (let i = 0; i < clean.length; i += 50) {
        const chunk = clean.slice(i, i + 50);
        this._write(`KM1${chunk};`);  // write to Text Memory 1
        this._write('KY6;');           // play back Text Memory 1 (Message Keyer 1)
      }
...
class RigctldClient extends EventEmitter {
  ...
  startTune() {
    if (!this.connected) return;
    if (this._yaesuRaw) {
      // Send only AC011; (standard Yaesu format) — sending multiple formats
      // causes ? errors that block the correct command
